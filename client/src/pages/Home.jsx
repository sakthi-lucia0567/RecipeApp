...
const Home = () => {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        const response = await axios.get("http://localhost:8000/recipe/all");
    ...
    const fetchSavedRecipes = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/recipe/getAllSavedRecipe/ids/${userID}`,
          {
            headers: {
  ...
  const handleSaveRecipe = async (recipeID) => {
    try {
      ...
      const response = await axios.put(
        "http://localhost:8000/recipe/saved-recipe",
        {
          userID,
          recipeID,
        },
      ...
      console.log("saved one", response);
      console.log("saved recipe", response.data.savedRecipe);
...
}
{
  /* <div className='flex flex-wrap justify-between'>
   This div is for future reference 

  <div className='flex space-x-2 text-sm dark:text-gray-400'>
     Comment Button For future use 
